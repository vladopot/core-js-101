...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const profNum = (x1 * x2) + (y1 * y2);
  const abs1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const abs2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  const costn = profNum / (abs1 * abs2);
  return Math.acos(costn);
}
...
function getParallelepipedDiagonal(a, b, c) {
  const firstDiag = (a ** 2) + (b ** 2);
  return Math.sqrt((firstDiag) + (c ** 2));
}
...
function toNumber(value, def) {
  ...
  if (type === 'undefined' || type === 'string' || type === 'Boolean' || Number.isNaN(val)) {
    result = def;
  } else if (type === 'number') {
    result = val;
  }
